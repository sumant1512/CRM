...
const getwallet = async (req, res) => {
  ...
  if(roleType != "superadmin" && roleType != "employee"){
    const getWalletsQuery =
      "SELECT * FROM expenses_managment.wallet ";
      try {
          connectDB.query(getWalletsQuery)
          .then(([result]) =>{
            if (result.length <=  0) {
              
              // return Promise.reject(new Error('Failed to delete expenses.'));
              res.status(500).send({ status: false, message: "Failed to fetch wallet data."});
              // Handle error or inform the user that the email already exists
            } else {
              res.status(200).send({ status: true, message: "Wallet data fetch succesfully.", data: result });
            }
          })
          .catch(err => {
              sendResponseError(500, "Error in fetching Wallet. Error- "+err.message,res);
  ...
  if(roleType != "superadmin" && roleType != "employee"){
    ...
    const addWalletQuery =
      ...
      try {
          ...
          connectDB.query(addWalletQuery,wallet_data)
          .then(([result]) =>{
            if (result.length <=  0) {
              
              // return Promise.reject(new Error('Failed to delete expenses.'));
              res.status(500).send({ status: false, message: "Failed to add wallet data."});
              // Handle error or inform the user that the email already exists
            } else {
              res.status(200).send({ status: true, message: "Wallet data added succesfully." });
            }
          })
          .catch(err => {
              sendResponseError(500, "Error in adding Wallet. Error- "+err.message,res);
  ...
  if(roleType != "superadmin" && roleType != "employee"){
    ...
    "UPDATE expenses_managment.wallet SET amount = ?,modified_at = NOW() WHERE id = ?";
      try {
          ...
          connectDB.query(updateWalletQuery,wallet_data)
          .then(([result]) =>{
            if (result.length <=  0) {
              
              // return Promise.reject(new Error('Failed to delete expenses.'));
              res.status(500).send({ status: false, message: "Failed to update wallet data."});
              // Handle error or inform the user that the email already exists
            } else {
              res.status(200).send({ status: true, message: "Wallet data updated succesfully." });
            }
          })
          .catch(err => {
              sendResponseError(500, "Error in updating Wallet. Error- "+err.message,res);
...
const getwalletById = async (req, res) => {
  ...
  if(roleType != "superadmin" && roleType != "employee"){
    const getWalletsQuery =
      ...
      try {
          connectDB.query(getWalletsQuery,[wallet_id])
          .then(([result]) =>{
            if (result.length <=  0) {
