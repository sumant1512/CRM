  ...
  try {
    ...
    if (payload) {
      const checkUserQuery =
        "SELECT COUNT(*) as count FROM expenses_managment.user WHERE id = ?";
      connectDB
        .query(checkUserQuery, [payload["id"]])
        .then(([rows]) => {
        ...
        .catch((err) => {
          sendResponseError(
            400,
            `Something went Wrong. Please check error - ` + err,
            res
          );
        });
    } else {
...
const incrementTransactionCount = (user_id, admin_id, res) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    const UserRoleCheck =
      "SELECT role_name, email FROM expenses_managment.user as users INNER JOIN expenses_managment.user_role as roles ON users.role_id = roles.id WHERE users.id= ?";
    const incrementTransCountQuery =
      "UPDATE expenses_managment.user SET transaction_count = transaction_count + 1,modified_at=NOW() WHERE id = ?";
    connectDB
      .query(UserRoleCheck, [user_id])
      .then(([result]) => {
        console.log(result);
        ...
        if (roleName == "superadmin") {
          ...
          return connectDB
            .query(incrementTransCountQuery, [admin_id])
            .then(([result]) => ({ roleName, result }));
        }
      })
